			...
			if ( preg_match( '/^[A-Za-z0-9]{16,32}$/', $token ) ) {
				...
				if ( is_array( $draft ) && $draft ) {
					...
					add_action( 'wp_footer', static function () {
						echo '<script id="icod-preview-only-js">(function(){var f=document.querySelector(".icod-form");if(!f){return;}var host=document.createElement("div");host.className="icod-preview-host";document.body.appendChild(host);host.appendChild(f);Array.prototype.forEach.call(document.body.children,function(el){if(el!==host&&el.tagName!=="SCRIPT"&&el.tagName!=="STYLE"&&el.id!=="wpadminbar"){el.style.display="none";}});var bar=document.getElementById("wpadminbar");if(bar){bar.style.display="none";}document.documentElement.classList.add("icod-preview-only");document.body.style.cssText+=";margin:0;padding:16px;background:#fff";window.scrollTo(0,0);})();</script>';
					} );
				}
			}
		}
