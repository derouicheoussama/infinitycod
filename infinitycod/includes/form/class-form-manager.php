			...
			if ( preg_match( '/^[A-Za-z0-9]{16,32}$/', $token ) ) {
				...
				if ( is_array( $draft ) && $draft ) {
					...
					add_action( 'wp_head', static function () {
						echo '<style id="icod-preview-style">html.icod-preview-only{margin-top:0!important;padding-top:0!important}html.icod-preview-only #wpadminbar{display:none!important}html.icod-preview-only body>*:not(.icod-preview-host):not(script):not(style):not(link){display:none!important}html.icod-preview-only body{margin:0!important;padding:16px!important}</style>'; // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedStylesheet -- isolation d'aperçu.
					} );
					add_action( 'wp_footer', static function () {
						echo '<script id="icod-preview-only-js">(function(){var f=document.querySelector(".icod-root")||document.querySelector(".icod-form");if(!f){return;}var host=document.createElement("div");host.className="icod-preview-host";document.body.appendChild(host);host.appendChild(f);Array.prototype.forEach.call(document.body.children,function(el){if(el!==host&&el.tagName!=="SCRIPT"&&el.tagName!=="STYLE"&&el.id!=="wpadminbar"&&el.tagName!=="LINK"){el.style.display="none";}});var bar=document.getElementById("wpadminbar");if(bar){bar.style.display="none";}document.documentElement.classList.add("icod-preview-only");document.body.style.cssText+=";margin:0;padding:16px";window.scrollTo(0,0);})();</script>';
					} );
				}
			}
		}
