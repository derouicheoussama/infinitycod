...
(function () {
	...
	function initForm(root) {
		...
		function updateHeadPrice() {
			...
			var badgeEl = el(root, '[data-head-discount]');
			if (badgeEl) {
				var disc = (regular > 0.01 && state.unitPrice > 0.01 && regular > state.unitPrice + 0.01) ? Math.round((1 - state.unitPrice / regular) * 100) : 0;
				badgeEl.textContent = '−' + disc + '%';
				badgeEl.classList.toggle('icod-hidden', disc <= 0);
			}
		}
		...
		var refreshQuote = debounce(function () {
			...
			api('quote', payload).then(function (json) {
				...
				/* Preuve sociale : cycle les vraies commandes recentes (donnees serveur). */
		var proofEl = el(root, '[data-social-proof]');
		if (proofEl) {
			var proofItems = [];
			try { proofItems = JSON.parse(proofEl.getAttribute('data-social-proof') || '[]'); } catch (e) { proofItems = []; }
			var proofIdx = 0;
			var showProof = function () {
				if (!proofItems.length) { return; }
				var it = proofItems[proofIdx % proofItems.length];
				proofIdx++;
				var box = document.createElement('div');
				box.className = 'icod-sp-toast';
				var t = document.createElement('span');
				t.className = 'icod-sp-txt';
				t.textContent = it.t;
				box.appendChild(t);
				var dot = document.createElement('span');
				dot.className = 'icod-sp-dot';
				box.appendChild(dot);
				document.body.appendChild(box);
				window.setTimeout(function () { box.classList.add('show'); }, 30);
				window.setTimeout(function () { box.classList.remove('show'); }, 6000);
				window.setTimeout(function () { box.remove(); }, 6500);
			};
			if (proofItems.length) {
				window.setTimeout(showProof, 5000);
				window.setInterval(showProof, 14000);
			}
		}
