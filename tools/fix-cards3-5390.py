# -*- coding: utf-8 -*-
"""Remplace le bloc corrompu (CRLF) par les 5 cartes HTML brutes."""
import io
...
# Le bloc corrompu commence au `"` parasite qui suit la fin de la carte SEO.
tail = "</label>" + CRLF + "\t\t\t</div>" + CRLF + "\t\t</div>" + CRLF + CRLF + DQ
start = d.find(tail, i_faq)
assert start > 0, "marker début introuvable"
start += len(tail)  # le `"` parasite = premier caractère du bloc corrompu

# Fin du bloc corrompu : début de la carte Devise propre (LF, sans échappements).
...
with io.open(p, "w", encoding="utf-8", newline="") as f:
    f.write(d)
print("OK bloc corrompu remplacé par les 5 cartes (HTML brut, LF)")
